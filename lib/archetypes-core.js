/** @typedef {{ id: number; nombre: string; esencia: string; frase_insignia: string; nota: string }} Archetype */
...
export const MOTIVATION_MATRIX = {
  aventura: [2, 21, 25],
...
/** @param {number} id */
function resolveArchetypeId(id) {
  if (id === 14) return 25;
  return ARCHETYPE_BY_ID.has(id) ? id : 1;
}
...
export function getArchetypeById(id) {
  ...
  return ARCHETYPE_BY_ID.get(resolved) ?? ARCHETYPE_BY_ID.get(1) ?? ARCHETYPES[0];
}

/** @param {Record<string, unknown>} profile */
function getPrimaryPace(profile) {
  const pace = asStringArray(profile.pace);
  return pace[0] ?? "balanced";
}

/** @param {Record<string, unknown>} profile */
function getSocial(profile) {
...
/** @param {Record<string, unknown>} profile */
function getSocialEnergy(profile) {
 ...
 * Always returns a valid Archetype (falls back to id 1).
 * @param {Record<string, unknown>} profile
 * @returns {Archetype}
 */
export function selectArchetype(profile) {
...
const PACE_MODIFIERS = {
  ...
  balanced: "Tono equilibrado, espacio para improvisar",
...
/** @param {Record<string, unknown>} profile */
export function getModifiers(profile) {
  const modifiers = [];
  const pace = getPrimaryPace(profile);
